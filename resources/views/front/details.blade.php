...
@section('title', 'Detail Toko | Cuci Mobil AquWash')
@section('content')
  <main class="bg-[#FAFAFA] max-w-[640px] mx-auto min-h-screen relative flex flex-col has-[#CTA-nav]:pb-[120px] has-[#Bottom-nav]:pb-[120px]">
    ...
    <section id="Thumbnail" class="relative">
      <div class="swiper h-fit">
        <div class="swiper-wrapper w-full h-fit">
          <div class="swiper-slide !w-[310px] !h-[350px] flex shrink-0 overflow-hidden">
            <img src="{{ Storage::url($carStore->thumbnail) }}" class="object-cover w-full h-full" alt="thumbnail">
          </div>
      ...
      <div class="px-4 flex justify-between items-center transform translate-y-1/2 absolute bottom-0 z-10 w-full">
        <p class="badge w-fit rounded-full p-[6px_10px] {{ $carStore->is_open ? 'bg-[#41BE64]' : 'bg-[#F12B3E]' }} font-bold text-xs leading-[18px] text-white">{{ $carStore->is_open ? 'OPEN NOW' : 'CLOSED' }}</p>
        <div class="flex items-center w-fit rounded-full border border-[#E9E8ED] p-[6px_12px] bg-white">
          ...
          <p class="text-sm leading-[21px] text-[#909DBF]"><span class="font-semibold text-[#270738]">4.8</span> <span>(145,394)</span></p>
        </div>
      </div>
    </section>
    <section id="details" class="flex flex-col gap-5 px-4 mt-[33px]">
      ...
      <div id="Menus" class="flex flex-col gap-5">
        <div class="tab-link-btns flex items-center gap-2">
          <button class="tablink rounded-full border border-[#E9E8ED] p-[8px_16px] font-semibold text-sm leading-[21px] transition-all duration-300 hover:bg-[#5B86EF] hover:text-white bg-[#5B86EF] text-white" onclick="openPage('about-tab', this)"  id="defaultOpen"><h2>About</h2></button>
          <button class="tablink rounded-full border border-[#E9E8ED] p-[8px_16px] font-semibold text-sm leading-[21px] transition-all duration-300 hover:bg-[#5B86EF] hover:text-white bg-white" onclick="openPage('reviews-tab', this)"><h2>Reviews</h2></button>
          <button class="tablink rounded-full border border-[#E9E8ED] p-[8px_16px] font-semibold text-sm leading-[21px] transition-all duration-300 hover:bg-[#5B86EF] hover:text-white bg-white" onclick="openPage('contact-tab', this)"><h2>Contact</h2></button>
        </div>
    ...
    <div id="CTA-nav" class="fixed bottom-0 w-full max-w-[640px] mx-auto border-t border-[#E9E8ED] flex items-center justify-between p-[16px_24px] bg-white z-20">
      ...
      @if ($carStore->is_open)
        @if ($carStore->is_full)
            <a href="{{ route('front.booking', $carStore->slug) }}" class="rounded-full p-[12px_20px] bg-[#FF8E62] font-bold text-white">Booking Now</a>
        @else

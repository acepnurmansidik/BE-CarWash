...
@section('title', 'Toko | Cuci Mobil AquWash')
@section('content')
  <main class="bg-[#FAFAFA] max-w-[640px] mx-auto min-h-screen relative flex flex-col has-[#CTA-nav]:pb-[120px] has-[#Bottom-nav]:pb-[120px]">
    <div id="Top-nav" class="flex items-center justify-between px-4 pt-5">
      ...
      <div class="flex flex-col w-fit text-center">
        ...
        <p class="text-sm leading-[21px] text-[#909DBF]">{{ $cityName }}, {{ $stores->count() }} Stores</p>
      </div>
    ...
    <section id="Store-list" class="flex flex-col gap-6 px-4 mt-[30px]">
        @forelse ($stores as $store)
            <a href="/store/details/{{ $store->slug }}" class="card">
                ...
                <div class="w-full h-[120px] flex shrink-0 overflow-hidden relative">
                    <img src="{{ Storage::url($store->thumbnail) }}" class="w-full h-full object-cover" alt="thumbnail">
                    <p class="rounded-full p-[6px_10px] {{ $store->is_open ? 'bg-[#41BE64]' : 'bg-[#F12B3E]' }} w-fit h-fit font-bold text-[10px] leading-[15px] text-white absolute top-4 right-4">{{ $store->is_open ? 'OPEN NOW' : 'CLOSED' }}</p>
                </div>
                <div class="flex items-center justify-between gap-4 px-4">
                    ...
                    <div class="flex items-center gap-[2px]">
                        ...
                        <p class="text-sm leading-[21px] text-[#909DBF]">{{ $store->addresss }}</p>
                    </div>
                    </div>

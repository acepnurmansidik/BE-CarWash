...
@section('title', 'Booking Detail | Cuci Mobil AquWash')
@section('content')
      ...
      <div class="flex flex-col w-full rounded-2xl border border-[#E9E8ED] p-4 gap-4 bg-white">
        <div id="Service" class="flex flex-col gap-2">
          <div class="flex items-center w-full gap-[10px] bg-white justify-between">
            ...
            <p class="rounded-full p-[6px_10px] {{ $details->is_paid ? 'bg-[#41BE64] text-white' : 'bg-[#FFCE51]' }} w-fit font-bold text-xs leading-[18px]">{{ $details->is_paid ? 'PAID' : 'PENDING' }}</p>
          </div>
        </div>
      </div>
    </div>
      ...
      <div class="flex flex-col w-full rounded-2xl border border-[#E9E8ED] p-4 gap-4 bg-white">
        <div id="Location" class="flex flex-col gap-2">
          ...
          <div class="flex items-center w-full gap-[10px] bg-white">
            <div class="w-[80px] h-[60px] flex shrink-0 rounded-xl overflow-hidden">
              <img src="{{ Storage::url($details->store_details->thumbnail) }}" class="w-full h-full object-cover" alt="thumbnail">
            </div>
            <div class="flex flex-col">
              ...
              <div class="flex items-center gap-[2px]">
                <p class="text-sm leading-[21px] text-[#909DBF]">{{ $details->store_details->addresss }}, {{ $details->store_details->city->name }}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div id="Price-details" class="flex flex-col gap-[10px]">
          <div class="flex items-center justify-between">
            ...
            <p class="font-semibold">Rp {{ number_format($details->service_details->price, 0, ',', '.') }}</p>
          </div>
          <div class="flex items-center justify-between">
          ...
          <div class="flex items-center justify-between">
            ...
            <p class="font-bold text-xl leading-[30px] text-[#FF8E62]">Rp {{ number_format($details->total_amount, 0, ',', '.') }}</p>
          </div>
        </div>
      </div>
    </div>
    <div class="px-8 mt-[30px] flex">
      <a href="tel:{{ $details->store_details->phone_number }}" class="w-full rounded-full p-[12px_20px] bg-[#FF8E62] font-bold text-white text-center">Call Customer Service</a>
    </div>
  </main>
@endsection